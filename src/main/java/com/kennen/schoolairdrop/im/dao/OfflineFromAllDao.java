...
public interface OfflineFromAllDao extends JpaRepository<OfflineFromAll, String> {
    ...
    @Query(nativeQuery = true, value = "select i1.* " +
            ...
            " where i1.receiver_id = ?2 and i1.sender_id in (select sender_id from offline_nums_detail_a?1 where receiver_id = ?2 union select sender_id from offline_nums_detail_b?1 where receiver_id = ?2)" +
