...
public class Redis {
     ...
     * @return true成功 false失败
     */
    private boolean setHashMap(String key, Map<String, Object> map, long time) {
        try {
            redisTemplate.opsForHash().putAll(key, map);
            if (time > 0) {
                expire(key, time);
     ...
     * @return true 存在 false不存在
     */
    public boolean isKeyValueExists(String key, Object value) {
        try {
     ...
     * @return
     */
    public long getSetLengthByKey(String key) {
        try {
     ...
     * @return
     */
    public boolean putList(String key, List<Object> value, long time) {
        try {
            redisTemplate.opsForList().rightPushAll(key, value);
            if (time > 0) expire(key, time);
            return true;
     ...
     * @return 移除的个数
     */
    public long removeValueByKey(String key, long count, Object value) {
        try {
